...
/* Lista encadeada de prontos (head/tail para append eficiente) */
static TCB *prontos_head = NULL;
static TCB *prontos_tail = NULL;

/* Ponteiro para o TCB atualmente executando (se houver) */
...
/* Remove um TCB da fila de prontos (se presente). Não desaloca memória. */
void scheduler_remover(TCB *tcb) {
    if (!prontos_head || !tcb) return;
    /* usa prox_pronto para operar a fila de prontos, preservando prox (lista global) */
    if (prontos_head == tcb) {
...
/* Inicializa o módulo scheduler com algoritmo e quantum */
...
/* Adiciona tcb à fila de prontos (append no tail) */
void scheduler_adicionar(TCB *tcb) {
    if (!tcb) return;
    /* ligar apenas na lista de prontos (prox_pronto) */
    tcb->prox_pronto = NULL;
    tcb->estado = PRONTA;
    if (!prontos_head) {
...
static TCB* choose_srtf(void) {
    ...
    for (TCB *p = prontos_head; p != NULL; p = p->prox_pronto) {
        if (!best || p->tempo_restante < best->tempo_restante) best = p;
    }
    if (executando && executando->estado == EXECUTANDO && executando->tempo_restante > 0) {
...
static TCB* choose_priority(void) {
    TCB *best = NULL;
    for (TCB *p = prontos_head; p != NULL; p = p->prox_pronto) {
        if (!best || p->tarefa.prioridade > best->tarefa.prioridade) best = p;
    }
    if (executando && executando->estado == EXECUTANDO && executando->tempo_restante > 0) {
...
/* Função pública: escolhe a próxima tarefa e a marca como executando.
 * - Remove o TCB da lista de prontos se ele estiver presente.
 * - Define executando = chosen.
 */
...
/* Retorna o TCB atualmente em execução */
TCB* scheduler_get_executando(void) {
    return executando;
}

/* Marca um TCB como finalizado:
 * - Remove da fila de prontos (caso esteja)
 * - Marca estado FINALIZADA e limpa ponteiro executando se for o caso.
 */
 ...
 *   para que o scheduler possa reconsiderá-la no próximo tick.
 * - Implementação simples: adiciona ao final da fila de prontos.
 */
void scheduler_yield_current(void) {
    ...
    if (t->estado == FINALIZADA) {
        /* não tocar em `prox` (lista global). apenas garantir que não esteja
           ligado na fila de prontos */
        t->prox_pronto = NULL;
        return;
    }
